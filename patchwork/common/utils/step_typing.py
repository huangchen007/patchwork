...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    import_path = f"{module_path}.typed"
    if import_path not in allowed_modules:
        raise ValueError(f"Disallowed module import: {import_path}")
    type_module = importlib.import_module(import_path)
